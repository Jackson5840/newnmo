...
function maketable(specval) {
  ...
  $.getJSON(apiurlbase + "/browse/region_name/" + specval, function(datalist) {     
...
$(function () {
  ...
  $("#selectregions").change(function () {
    specval = document.getElementById("selectregions").value;
    maketable(specval);
  })
  ...
  function loadList(){
    $.getJSON(apiurlbase + "/metavals/?fields=region", function(labsList) {             
      var output = [];  
      $.each(labsList[0].vals, function(key,value)
        ...
        $('#selectregions').html(output.join(''));
