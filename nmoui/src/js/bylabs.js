...
function maketable(specval) {
  ...
  $.getJSON(apiurlbase + "/browse/archive_name/" + specval, function(datalist) {     
...
$(function () {
  ...
  $("#selectlabs").change(function () {
    specval = document.getElementById("selectlabs").value;
    maketable(specval);
  })
  ...
  function loadList(){
    $.getJSON(apiurlbase + "/metavals/?fields=archive_name", function(labsList) {             
      var output = [];  
      $.each(labsList[0].vals, function(key,value)
        ...
        $('#selectlabs').html(output.join(''));
        $('#selectlabs').selectpicker('refresh');
