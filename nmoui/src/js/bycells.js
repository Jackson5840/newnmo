...
function maketable(specval) {
  ...
  $.getJSON(apiurlbase + "/browse/celltype_name/" + specval, function(datalist) {
...
$(function () {
  ...
  $("#selectcells").change(function () {
    specval = document.getElementById("selectcells").value;
    maketable(specval);
  })
  ...
  function loadList(){
    $.getJSON(apiurlbase + "/metavals/?fields=celltype", function(labsList) {             
      var output = [];  
      $.each(labsList[0].vals, function(key,value)
        ...
        $('#selectcells').html(output.join(''));
        $('#selectcells').selectpicker('refresh');
